...
<script>
    ...
    let bgColor = $derived(
        isSelected ? (
            status === 'completed' ? 'bg-moss_green-300' :
            status === 'registered' ? 'bg-thistle-300' :
            status === 'overdue' ? 'bg-redwood-300' :
            'bg-amber-300'
        ) : (
            status === 'completed' ? 'bg-moss_green' :
    ...
    let hoverColor = $derived(
        isSelected ? (
            status === 'completed' ? 'hover:bg-moss_green-200' :
            status === 'registered' ? 'hover:bg-thistle-200' :
            status === 'overdue' ? 'hover:bg-redwood-200' :
            'hover:bg-amber-200'
        ) : (
            status === 'completed' ? 'hover:bg-moss_green-400' :
