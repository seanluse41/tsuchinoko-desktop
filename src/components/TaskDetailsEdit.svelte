...
<script>
    ...
    let { task, formData, onSave, onCancel } = $props();
    ...
    function initializeDatepicker() {
        if (dateInput) {
            ...
            datepicker = new AirDatepicker(dateInput, {
                ...
                inline: false,
                position: "top left",
                selectedDates: formData.dateDue ? [new Date(formData.dateDue)] : [],
                onSelect({ date }) {
...
<div class="space-y-6 max-w-2xl">
    <div>
        <Label for="task-name" class="font-bold text-slate-700">{$_("taskDetail.name")}</Label>
        <Input 
            id="task-name"
            type="text"
    ...
    <div>
        <Label for="task-due" class="font-bold text-slate-700">{$_("taskDetail.due")}</Label>
    ...
    <div>
        <Label for="task-folder" class="font-bold text-slate-700">{$_("taskDetail.folder")}</Label>
        <Select
            id="task-folder"
    ...
    <div>
        <Label for="task-description" class="font-bold text-slate-700">{$_("taskDetail.description")}</Label>
        <Textarea
            id="task-description"
    ...
    <div>
        <Label for="task-memo" class="font-bold text-slate-700">{$_("taskDetail.memo")}</Label>
        <Textarea
            id="task-memo"
    ...
    {#if formData.status === 'completed'}
        <div>
            <Label for="task-completion-memo" class="font-bold text-slate-700">{$_("taskDetail.completionNotes")}</Label>
            <Textarea
