<script>
    ...
    const alarmSounds = [
        { id: 'notificationTimer', label: 'Notification', file: 'notificationTimer.mp3' },
    ...
    onMount(() => {
        ...
        $effect(() => {
            if (timerState.isExpired && !timerState.alarmPlayed && alarmAudio && isSoundEnabled) {
...
<div class="flex flex-col md:flex-row md:gap-8 items-start py-8 px-2 md:p-8">
    ...
    <div class="hidden md:block md:w-48 lg:w-64">
        <p class="mb-4 font-semibold text-slate-700">
            Alarm Sound: <span class="capitalize">{timerState.alarmSound.replace('.mp3', '')}</span>
        </p>
        
        {#if !isSoundEnabled}
            <div class="p-3 bg-amber-50 border border-amber-200 rounded-lg mb-4 text-amber-700 text-sm">
                Alarm sounds are disabled in preferences
            </div>
        {/if}
        
        <ul
            class="w-full divide-y divide-gray-200 rounded-lg border border-gray-200 bg-white {!isSoundEnabled ? 'opacity-50' : ''}"
        >
        ...
        <!-- Loop checkbox -->
        <div class="mt-4 p-3 bg-white border border-gray-200 rounded-lg {!isSoundEnabled ? 'opacity-50' : ''}">
            <Checkbox 
    ...
    <!-- Main timer display -->
    <div class="flex flex-col items-center justify-center gap-4 md:gap-8 w-full md:flex-1">
        <!-- Timer display -->
        <div 
        ...
        <div class="mt-6 md:hidden w-full">
            ...
            {:else}
                ...
                <div class="grid grid-cols-2 gap-2 bg-white rounded-lg border border-gray-200 p-2">
                    {#each alarmSounds as sound}
                        <div class="p-1">
                            <Radio 
                                name="alarm-sound" 
